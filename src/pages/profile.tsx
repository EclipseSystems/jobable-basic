						...
						<FieldSet>
							<FieldGroup>
								...
								<Field className={'w-30'}>
									<Button className={'cursor-pointer'} type={'submit'}><Save /> Save</Button>
								</Field>
							</FieldGroup>
						</FieldSet>
					</CardContent>
							...
							<TableBody>
								{methods.map((method) => (
									<TableRow>
										...
										<TableCell>
											<Badge variant={method.status === 'Enabled' ? 'default' : 'outline'}>{method.status}</Badge>
										</TableCell>
										<TableCell>
											<Button variant={method.button === 'Enable' ? 'outline' : 'destructive'} size={'sm'}>{method.button}</Button>
										</TableCell>
									</TableRow>
								))}
							</TableBody>
